...
- Never bump `version`/`appVersion` in `charts/zabbix/Chart.yaml` or `charts/zabbix/artifacthub-pkg.yml` yourself — that's part of the maintainers' release process, done via git tag (`zabbix-x.y.z` triggers `.github/workflows/helm-chart-releaser.yml`).
...
- The maintainer-only release steps are documented twice, and both need to stay consistent if the process changes: `CONTRIBUTING.md` ("For code mainteners only" section, exact commands) and the root `README.md` ("Release process" section, a summary plus a Mermaid flowchart of the same steps).
